...
def quality_type(value):
    """Custom type for quality arguments that supports both full and short names"""
    quality_map = {
        'l': 'low',
        'm': 'medium', 
        'h': 'high',
    ...
    if value in quality_map:
        ...
        raise argparse.ArgumentTypeError(f"Invalid quality level: {value}. "
                                       f"Choose from: l/low, m/medium, h/high, p/2k, k/4k")


def main():
    parser = argparse.ArgumentParser(description="Maniq - Manim Quality Stress Testing Tool")
    
    # Add version arguments (both long and short)
    parser.add_argument('--version', '-V', action='version', version=f'maniq {__version__}')
    ...
    if not args.code_dir:
        if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] in ['--version', '-V', '-h', '--help']):
            parser.print_help()
            return
        else:
